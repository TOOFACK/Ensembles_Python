...
class Bagger:
    ...
    def fit(self, X, y):
        ...
        self.indices = []
        for i in range(self.n_estimators):
            indices = self.object_sampler.sample_indices(X.shape[0])
            x, y1 = X[indices], y[indices]
    ...
    def predict_proba(self, X):
        ...
        for f_inds, estimator in zip(self.indices, self.estimators):
            current_proba = estimator.predict_proba(X[:, f_inds])
