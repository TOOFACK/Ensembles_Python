...
class Booster:
    ...
    def _fit_first_estimator(self, X, y):
        ...
        f_indices = self.feature_sampler.sample_indices(X.shape[1])
        x = X[:, f_indices]

        base_est = self.base_estimator(**self.params)
        base_est.fit(x, y)
        self.estimators.append(base_est)
        self.indices.append(f_indices)
    ...
    def predict(self, X):
        ...
        preds = self.estimators[0].predict(X[:, self.indices[0]])

        for i in range(1, self.n_estimators):
            preds += self.weights[i] * self.estimators[i].predict(X[:, self.indices[i]])
...
class GradientBoostingClassifier(Booster):
    ...
    def _loss(self, y_true, y_pred):
        ...
        Calculate NLL
        """
        # print(np.log(1/expit(-y_pred)))
        # print(np.log(1 + np.exp(y_pred)))
        return - y_true * y_pred + np.log(1/expit(-y_pred))
    ...
    def _fit_base_estimator(self, X, y, predictions):
        ...
        def f(a):
            return np.sum(self._loss(y, predictions + a * curr_est.predict(x)))

        # print("Y   = ", y)
        curr_grad = self._gradient(y, predictions)
        f_indices = self.feature_sampler.sample_indices(X.shape[1])
        x = X[:, f_indices]
        self.indices.append(f_indices)
        curr_est = self.base_estimator(**self.params)
        curr_est.fit(x, -curr_grad)
        self.estimators.append(curr_est)
        self.weights.append(minimize_scalar(f).x)
